...
void _insert(std::vector<T>& arr, size_t curr, size_t target, size_t size) {
  ...
  target -= size;
  size_t i = 0;
  for (size_t idx = target; idx <= curr + size; idx++) {
    temp[i] = arr[idx];
    arr[idx] = arr[curr + i];
    i = i > size - 1 ? i + 1 : 0;
  }
...
size_t _divideConquer(std::vector<T>& arr, std::vector<size_t> main_seq,
                      size_t idx) {
  size_t standard = idx / 2;
  size_t pos = standard;
  while (standard > 1) {
    standard /= 2;
    if (arr[main_seq[idx]] < arr[main_seq[pos]]) {
      pos += standard;
    } else if (arr[main_seq[idx]] > arr[main_seq[pos]]) {
      pos -= standard;
    } else
      break;
  }
...
void merge_insert_sort(std::vector<T>& arr, size_t rank) {
  ...
  for (size_t front = rank - 1; front < arr.size(); front += 2 * rank) {
  ...
  if (main_seq.size() > 2) {
    merge_insert_sort(main_seq, rank * 2);
  }
  ...
  for (size_t idx = 0; idx < main_seq.size(); idx++) {
    size_t pos = _divideConquer(arr, main_seq, idx);
    _insert(arr, main_seq[idx], main_seq[pos], rank);
  }
}
