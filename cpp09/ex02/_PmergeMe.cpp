...
void PmergeMe::_insert(size_t rank, size_t target_idx, size_t curr_idx) {
  // size_t curr = 0;
  // std::vector<unsigned int> back(size - target_idx + 2);
  // std::vector<unsigned int> front(rank);
  // while (curr < rank) {
  //   }
  ...
  //   curr++;
  (void)rank;
  unsigned int temp = _arr[curr_idx];
  if (curr_idx < target_idx) {
    // Move element towards the end, adjust for shifted indices
    for (size_t i = curr_idx; i < target_idx - 1; ++i) {
      _arr[i] = _arr[i + 1];
    }
  } else if (curr_idx > target_idx) {
    // Move element towards the beginning
    for (size_t i = curr_idx; i > target_idx; --i) {
      _arr[i] = _arr[i - 1];
    }
  }
  // Place the element at the target position
  _arr[target_idx] = temp;  // Assuming temp holds the value to be moved
}
